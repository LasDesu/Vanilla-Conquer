...
void* Get_Shape_Header_Data(void* ptr)
{
    if (UseBigShapeBuffer) {
        ...
        return (
            (void*)((uintptr_t)header->shape_data + (uintptr_t)(header->shape_buffer ? TheaterShapeBufferStart : BigShapeBufferStart)));
    ...
    if (UseBigShapeBuffer) {
        ...
        */
        if (keyfr->x != UNCOMPRESS_MAGIC_NUMBER) {
            ...
            */
            ...
            memset(KeyFrameSlots[keyfr->y], 0, keyfr->frames * sizeof(KeyFrameSlots[0]));
        }
        ...
        */
        if (*(KeyFrameSlots[keyfr->y] + framenumber)) {
            if (IsTheaterShape) {
                return ((uintptr_t)TheaterShapeBufferStart
                        + (uintptr_t)*(KeyFrameSlots[keyfr->y] + framenumber));
            } else {
